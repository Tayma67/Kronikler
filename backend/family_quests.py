...
CHILD_QUESTS = [
    ...
    # ── GEÇİŞ ARKİ (11-12 yaş zinciri) ──────────────────────────────────────
    # Bu 3 quest + yetiskinlige_hazirlik birbirine bağlı bir anlatı oluşturur.
    # "requires" alanı: bir önceki quest bitmeden bu açılmaz.
    {
        "id": "usta_sinavi",
        "title": "Ustanın Yanında",
        "giver_role": "baba",
        "min_age": 11,
        "requires": None,          # ark başlangıcı, bağımsız
        "description": (
            "Baban seni köyün en iyi zanaatkârına gönderiyor: "
            "'Bir usta yanında ter dökmeden adam olamazsın.' "
            "Köylü olarak beş gün çalış — ustanın gözü üzerinde."
        ),
        "objective": {"type": "work_count", "qty": 5},
        "reward": {
            "money": 20,
            "stat_xp": {"strength": 18, "stamina": 20, "intelligence": 10},
            "skill_xp": {"crafting": 15, "combat": 5},
            "flavor": (
                "Usta ellerini kuruluyor: 'Pek de kötü değilsin. "
                "Bir gün belki adam olursun.' Baban gururla gülümsüyor."
            ),
        },
    },
    {
        "id": "kasabada_tek",
        "title": "Kasabada Yalnız",
        "giver_role": "anne",
        "min_age": 11,
        "requires": "usta_sinavi",   # ustanın yanında bitmeden açılmaz
        "description": (
            "Annen seninle dürüst: 'Artık beni dinle — köyün dışını bilmeden "
            "büyüyemezsin.' Seni ilk kez yalnız kasabaya gönderiyor. "
            "Başka bir lokasyona git ve orada biriyle konuş."
        ),
        "objective": {"type": "travel_then_chat", "travel_qty": 1, "chat_qty": 1,
                      "travel_done": False, "chat_done": False},
        "reward": {
            "money": 10,
            "stat_xp": {"charisma": 20, "intelligence": 18},
            "skill_xp": {"social": 15, "trade": 8},
            "flavor": (
                "Eve döndüğünde annen kapıda bekliyordu. "
                "'Nasıldı?' dedi. 'Büyük' dedin. İkisi de anladı ne demek istediğini."
            ),
        },
    },
    {
        "id": "son_cocukluk",
        "title": "Son Çocukluk Gecesi",
        "giver_role": "anne",
        "min_age": 12,
        "requires": "kasabada_tek",  # kasabada_tek bitmeden açılmaz
        "description": (
            "Annen bu gece seni özel bir şeye çağırıyor. "
            "'Yarın 13'üne giriyorsun — bu son çocukluk gecen.' "
            "Önce annene git (onunla konuş), sonra babanla da bir konuş. "
            "Ailenle bu geceyi geçir."
        ),
        "objective": {"type": "chat_count", "qty": 4},
        "reward": {
            "money": 0,
            "stat_xp": {"charisma": 15, "intelligence": 20, "stamina": 10},
            "skill_xp": {"social": 12},
            "flavor": (
                "Baban ateşin başında eski bir hikâye anlattı — savaşlardan, "
                "dostlardan, kaybedilenlerden. Annen sessizce ağladı. "
                "Sen de bir şeyler hissettin ama ne olduğunu tarif edemiyordun. "
                "Sabah her şey farklı olacaktı."
            ),
        },
    },
    # ── ARK SONU (capstone — bağımsız, min_age:12) ──────────────────────────
    {
        "id": "yetiskinlige_hazirlik",
        "title": "Yetişkinliğe Hazırlık",
        "giver_role": "baba",
        "min_age": 12,
        "requires": None,          # bağımsız — son_cocukluk'tan ayrı yol
...
def make_family_quests(state, mother_id, father_id):
    ...
    for q in CHILD_QUESTS:
        ...
        obj = dict(q["objective"])  # kopya al, travel_then_chat için mutable
        quests.append({
            "id": q["id"],
            "title": q["title"],
            ...
            "min_age": q["min_age"],
            "requires": q.get("requires"),   # ark zinciri
            "objective": obj,
            "reward": q["reward"],
            "progress": 0,
...
def unlock_age_appropriate(state):
    """Mark quests as 'açık' when player reaches min_age AND prerequisite is done.
    Returns list of newly unlocked quests.
    """
    from calendar_tr import player_age
    age = player_age(state)
    completed_ids = {q["id"] for q in state.get("family_quests", []) if q["status"] == "tamamlandı"}
    newly = []
    for q in state.get("family_quests", []):
        ...
        if age < q["min_age"]:
            continue
        req = q.get("requires")
        if req and req not in completed_ids:
            continue  # ark zinciri tamamlanmadı
        q["status"] = "açık"
        newly.append(q)
    return newly
...
def progress_quest(state, action_type, payload=None):
    ...
    for q in state.get("family_quests", []):
        ...
        elif otype == "travel_then_chat":
            # Seyahat + sohbet kombinasyonu (kasabada_tek qusti için)
            if action_type == "travel" and not obj.get("travel_done"):
                obj["travel_done"] = True
            if action_type == "chat" and obj.get("travel_done") and not obj.get("chat_done"):
                obj["chat_done"] = True
            steps = (1 if obj.get("travel_done") else 0) + (1 if obj.get("chat_done") else 0)
            q["progress"] = steps
            if obj.get("travel_done") and obj.get("chat_done"):
                done = True

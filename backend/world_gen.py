...
def _assign_crushes(npcs):
    ...
    Kıskaçlık sisteminin işlemesi için gerekli başlangıç verisi.
    """
    ...
    for npc in singles:
        if random.random() > 0.15:
            continue
        # Aynı bölgede, farklı cinsiyette, bekar biri
        candidates = [
            c for c in singles
            if c["id"] != npc["id"]
            and c["location_id"] == npc["location_id"]
            and c.get("gender") != npc.get("gender")
            and c.get("crush_on") != npc["id"]  # karşılıklı olmak zorunda değil
        ]
        if candidates:
...
def generate_world(n_kingdoms=3, n_cities=3, n_villages=10, n_castles=5, n_npcs=100):
    ...
    _link_family(npcs)
    _assign_crushes(npcs)  # Kıskaçlık sistemi için crush_on ata

    # ── Şehir Yönetimi: Her lokasyon için governance objesi oluştur ────────
    from city_governance import make_governance, _find_governor_for_location
    # _find_governor_for_location state["world"] yapısı bekliyor
...
def generate_player(world, name=None, gender=None, surname=None):
    ...
    player = {
        ...
        "education": "yok",
        "reputation": 0,
        "honor": 0,       # şeref — onurlu davranışlarla artar
        "fear": 0,        # korku — saldırı/cinayet/kötü şöhretle artar
        "fame": 0,        # ün — kahramanlık/kamuoyuyla artar
        "health": 100,
        "hunger": 100,
        "crime": 0,
        "location_id": location["id"],

  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg, paddingTop: insets.top }}>
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 12 }}>
        ...
        <Text style={{ fontFamily: F.display, fontSize: 16, color: C.parchment, letterSpacing: 1 }}>{t("scr.mektep")}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: insets.bottom + 80 }}>
        {SUBJECTS.map((sub) => (
          <Pressable key={sub.id} onPress={() => apply((s) => studySubject(s, sub.id))} disabled={p.dead} style={{ flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: C.card, borderWidth: 1, borderColor: C.border, borderRadius: 10, padding: 14, marginBottom: 9 }}>
            ...
            <Text style={{ fontFamily: F.display, fontSize: 11, color: C.gold }}>{t("mek.study")}</Text>
          </Pressable>
        ))}
      </ScrollView>
    </View>

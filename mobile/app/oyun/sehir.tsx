    ...
    <View style={{ flex: 1, backgroundColor: C.bg, paddingTop: insets.top }}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: insets.bottom + 80 }}>
        ...
        {LOCATIONS.map((loc) => {
          const cur = loc === here;
          return (
            <Pressable key={loc} onPress={() => router.push(`/oyun/diyar/${encodeURIComponent(loc)}`)} style={{ flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: C.card, borderWidth: 1, borderColor: cur ? "rgba(201,168,76,0.5)" : C.border, borderLeftWidth: cur ? 2.5 : 1, borderLeftColor: cur ? C.gold : C.border, borderRadius: 10, padding: 14, marginBottom: 8 }}>

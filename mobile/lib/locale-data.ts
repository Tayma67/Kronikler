...
export const NAME_POOLS: Record<Lang, { m: string[]; f: string[]; s: string[] }> = {
  tr: {
    ...
    f: ["Ayşe","Fatma","Zeynep","Emine","Hatice","Elif","Nur","Reyhan","Cansu","Derya","Sevda","Pınar","Gül","Nazlı","Hande","Melek","Gülbahar","Sultan"],
    s: ["Atay","Bircan","Demirhan","Saygı","Açıkel","Dalkılıç","Kayhan","Bal","Yıldırım","Toprak","Çelik","Aydın","Korkmaz","Şahin"],
  },
  en: {
    m: ["William","John","Robert","Edmund","Henry","Thomas","Walter","Hugh","Roger","Geoffrey","Richard","Simon","Gilbert","Ralph","Edward","Osbert","Alan","Miles"],
    f: ["Alice","Maud","Agnes","Edith","Joan","Margery","Cecily","Isabel","Eleanor","Matilda","Beatrice","Rose","Emma","Aveline","Sybil","Avice","Clemence","Sabina"],
    s: ["Ashford","Blackwood","Carter","Smith","Marsh","Thatcher","Fletcher","Hale","Stone","Wells","Reeve","Carver","Holt","Frost"],
  },
  es: {
    m: ["Juan","Pedro","Diego","Rodrigo","Alfonso","Fernando","Gonzalo","Ramiro","Sancho","Martín","Lope","García","Bermudo","Nuño","Álvaro","Íñigo","Ordoño","Vela"],
    f: ["María","Inés","Leonor","Urraca","Blanca","Constanza","Teresa","Elvira","Sancha","Mencía","Catalina","Isabel","Aldonza","Jimena","Beatriz","Estefanía","Toda","Milia"],
    s: ["del Valle","Herrero","Ríos","Castro","Vega","Montes","Salazar","Cuéllar","Bravo","Acero","Aguilar","Pardo","Sierra","León"],
  },
  pt: {
    m: ["João","Pedro","Diogo","Rodrigo","Afonso","Fernão","Gonçalo","Vasco","Sancho","Martim","Lopo","Garcia","Nuno","Álvaro","Egas","Mendo","Paio","Soeiro"],
    f: ["Maria","Inês","Leonor","Urraca","Branca","Constança","Teresa","Elvira","Sancha","Mécia","Catarina","Isabel","Aldonça","Mor","Beatriz","Dórdia","Ouroana","Châmoa"],
    s: ["do Vale","Ferreiro","Rios","Castro","Veiga","Montes","Salazar","Bravo","Aço","Aguiar","Pardo","Serra","Leão","Pinto"],
  },
  ar: {
    m: ["محمد","أحمد","علي","حسن","حسين","إبراهيم","عثمان","يوسف","عمر","خالد","سليمان","داود","يعقوب","صلاح","طارق","حمزة","أيوب","بلال"],
    f: ["فاطمة","عائشة","زينب","مريم","خديجة","ليلى","نور","رحمة","سعاد","هند","سارة","أمينة","حليمة","نادية","سلمى","نفيسة","زبيدة","صفية"],
    s: ["الحدّاد","النجّار","التاجر","الصيّاد","الراعي","الخبّاز","القيسي","الدمشقي","الحلبي","المصري","الأندلسي","الفارسي","الكردي","البغدادي"],
  },
  ru: {
    m: ["Иван","Пётр","Фёдор","Дмитрий","Алексей","Михаил","Никита","Борис","Глеб","Ярослав","Святослав","Олег","Игорь","Роман","Владимир","Мстислав","Добрыня","Всеволод"],
    f: ["Мария","Анна","Ольга","Дарья","Евдокия","Прасковья","Ксения","Варвара","Елена","Татьяна","Анастасия","Софья","Марфа","Наталья","Любовь","Василиса","Забава","Милослава"],
...
const QUIRKS_L: Record<Lang, string[]> = {
  tr: QUIRKS_TR,
  en: ["always complains about the weather","loves telling of the old days","gives everyone nicknames","speaks little, listens much","laughs out loud","loves to haggle","gossips about the neighbors","hums a folk tune","peppers every remark with a proverb","keeps raisins in a pocket to hand out to children","tells the hour by a shadow, never wrong","greets everyone by name","crumbles bread for every bird in sight","sweeps the doorstep twice a day"],
  es: ["siempre se queja del clima","le encanta hablar de los viejos tiempos","les pone apodos a todos","habla poco y escucha mucho","ríe a carcajadas","le encanta regatear","critica a sus vecinos","tararea una tonada","adereza cada frase con un refrán","lleva pasas en el bolsillo para repartir a los niños","sabe la hora por la sombra, sin fallar","saluda a todos por su nombre","desmiga pan para cada pájaro que ve","barre su umbral dos veces al día"],
  pt: ["queixa-se sempre do tempo","adora falar dos velhos tempos","dá alcunhas a toda a gente","fala pouco e ouve muito","ri alto","adora regatear","fala mal dos vizinhos","cantarola uma toada","tempera cada frase com um provérbio","traz passas no bolso para dar às crianças","sabe as horas pela sombra, sem falhar","cumprimenta todos pelo nome","esmigalha pão para cada pássaro que vê","varre a soleira duas vezes ao dia"],
  ar: ["يشكو الطقس دائمًا","يحبّ الحديث عن الأيّام الخوالي","يطلق الألقاب على الجميع","يقلّ كلامه ويكثر إصغاؤه","يضحك بصوت عالٍ","يعشق المساومة","يغتاب جيرانه","يدندن أغنية شعبيّة","يقرن كلامه دائما بمثل","يحمل زبيبا في جيبه يوزعه على الصغار","يعرف الوقت من الظل ولا يخطئ","يسلم على الجميع بأسمائهم","يفتت الخبز لكل طائر يراه","يكنس عتبته مرتين في اليوم"],
  ru: ["вечно жалуется на погоду","любит рассказывать о былом","всем раздаёт прозвища","мало говорит, много слушает","громко смеётся","обожает торговаться","судачит о соседях","напевает народную песню","к каждому слову прибавляет пословицу","носит в кармане изюм и раздаёт его детям","по тени безошибочно узнаёт час","здоровается с каждым по имени","крошит хлеб каждой встречной птице","метёт свой порог дважды в день"],
};
const GOALS_L: Record<Lang, string[]> = {
  tr: GOALS_TR,
  en: ["dreams of opening a shop","wants to marry off a child","is trying to clear debts","wishes to go on pilgrimage","is saving to buy land","wants to become a foreman","wants to reconcile with an estranged sibling","is set on building a house","wants to take up caravan trade","wants to make a name","longs to see the homeland not seen for years","wants to see the apprentice set up with a shop","wants to bring the family vineyard back to life","wants to take in an orphan as an apprentice"],
  es: ["sueña con abrir una tienda","quiere casar a un hijo","intenta saldar sus deudas","desea peregrinar","ahorra para comprar tierra","quiere ser capataz","quiere reconciliarse con un hermano distanciado","se empeña en construir una casa","quiere lanzarse al comercio de caravanas","quiere hacerse un nombre","anhela ver su tierra natal tras muchos años","quiere ver a su aprendiz con tienda propia","quiere revivir la viña heredada de su padre","quiere acoger a un huérfano como aprendiz"],
  pt: ["sonha em abrir uma loja","quer casar um filho","tenta saldar as dívidas","deseja peregrinar","poupa para comprar terra","quer tornar-se capataz","quer reconciliar-se com um irmão afastado","está empenhado em construir uma casa","quer lançar-se no comércio de caravanas","quer fazer um nome","anseia por ver a terra natal após muitos anos","quer ver o aprendiz com loja própria","quer fazer reviver a vinha herdada do pai","quer acolher um órfão como aprendiz"],
  ar: ["يحلم بفتح دكّان","يريد تزويج ابنه/ابنته","يحاول سداد ديونه","يتمنّى الحجّ","يدّخر لشراء أرض","يريد أن يصير رئيس عمّال","يريد الصلح مع أخٍ مقاطع","عازم على بناء بيت","يريد خوض تجارة القوافل","يريد أن يُشتهَر","يشتاق إلى رؤية بلدته التي لم يرها منذ سنين","يريد أن يرى صبيه صاحب دكان","يريد إحياء كرم ورثه عن أبيه","يريد أن يضم يتيما صبيا عنده"],
  ru: ["мечтает открыть лавку","хочет женить/выдать замуж дитя","пытается расплатиться с долгами","желает совершить паломничество","копит на покупку земли","хочет стать старшим мастером","хочет помириться с отдалившимся родичем","намерен построить дом","хочет заняться караванной торговлей","хочет прославить своё имя","тоскует по родным краям, не виданным годами","хочет увидеть подмастерье с собственной лавкой","хочет возродить виноградник, доставшийся от отца","хочет взять сироту в подмастерья"],
};

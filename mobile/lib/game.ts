...
export const MARKET_EVENTS: { key: string; goods: string[]; mult: number; months: number; text: string }[] = [
  ...
  { key: "peynirmev", goods: ["peynir", "corba"], mult: 0.75, months: 2, text: "Yayla dönüşü: tulum peynir bol, kazanlar coştu — fiyat gevşedi." },
  { key: "gemiyatti", goods: ["balik", "et"], mult: 1.35, months: 2, text: "Fırtına iskeleyi dövdü, tekneler bağlı kaldı: balık ve et pahalandı." },
];
...
function rollLifeEvents(s: GameState, cal: CalendarInfo) {
  ...
  if (!p.dead && p.pet && p.dog && !inJail(p) && chance(0.03)) {
    ...
    if (dr < 0.34 && cal.season === "Kış") push(s, "gunluk", `Kış gecesi közün başında ${p.dog.n} boylu boyunca serildi; ${p.pet.n} hiç tereddüt etmeden sırtına kıvrıldı. Kapının bekçisiyle ocağın sultanı tek yürek gibi soluyor.`, "kişisel", false, { k: "evj.duo1", p: [p.pet.n, p.dog.n] });
    else if (dr < 0.34) push(s, "gunluk", `Öğle sıcağında ${p.dog.n} dut ağacının gölgesine serildi; ${p.pet.n} biraz ötede, gölgenin en serin karışını çoktan kapmıştı. İkisi de tek gözü açık uyuyor.`, "kişisel", false, { k: "evj.duo4", p: [p.pet.n, p.dog.n] });
...
const WORLD_NEWS: string[] = [
  ...
  "%b'de bir çırak ustasının ölümünden sonra tezgâhı devraldı; ilk işini bedava yaptı, 'ustamın borcu' dedi — %b2 çarşısında bile konuşuldu.",
  "%b ile %b2 arasında bir köprü çöktü; iki beyliğin taşçıları ilk kez omuz omuza çalışıyor, kavga bir yana bırakıldı.",
];
...
export function opportunitiesFor(s: GameState): Opportunity[] {
  const pool: Omit<Opportunity, "id">[] = [
    ...
    { key: "buz_kesimi", minAge: 14, title: "Buz Kesimi", desc: "Kış gölünden konak kileri için buz kes.", reward: 36, risk: 0.45, stat: "strength" },
    { key: "panayir_tellali", minAge: 13, title: "Panayır Tellallığı", desc: "Panayır boyunca tellallık et, kalabalığı meydana topla.", reward: 28, risk: 0.3, stat: "charisma" },
  ];
...
const MICRO_R_TR: Record<string, [string, string]> = {
  ...
  berber_aynasi: [
    "Berber aynayı arkana tuttu; bir an kendine yabancı gibi baktın: ne zaman büyümüşsün? Tıraş bitti, düşünce sürdü.",
    "Berber dükkânında iki laf, üç dedikodu; çıkarken yanağın da gönlün de ferahlamıştı. Kimi ustanın makası dile de biçki atar.",
  ],
  degirmen_sirasi: [
    "Değirmen sırasında önündeki ihtiyar 'benimki az, sen geç' dedi; sıra bir anda insanlığa döndü. Un öğütüldü, gönül de.",
    "Sıra beklerken değirmencinin çırağıyla laflayıp taşın dilini öğrendin; bir daha un alırken kulağın çarkta olacak.",
  ],
  dere_bendi: [
  ...
  salincak_ipi: [
    "Dala tırmanıp ipi sağlam bağladın; ilk sallanış hakkı senin oldu. Çınar bir yaz boyu gıcırdadı, kimse düşmedi.",
    "Kayıkçı düğümünü aşağıda gösterdin; ip dalda ilk denemede tuttu. Artık her salıncak ipini sana getiriyorlar.",
  ],
  cember_cevirme: [
    "Değneği çemberin içine tam açıyla vurdun; çember sokağın sonuna dek yuvarlandı, ardından koştun. Rüzgâr kulağında, dünya çemberin içinde.",
    "Küçük komşu çocuğuna çevirmeyi öğrettin; üçüncü denemede tutturdu, sevinci sokağı doldurdu. Öğretmek de bir oyunmuş.",
  ],
  asik_oyunu: [
    "Aşığı tam 'cik' diye attın; sıra dizilen aşıkları bir vuruşta topladın. Cebin doldu, arkadaşların hayran.",
    "Kaybedince tek aşığını da verdin, homurdanmadın; ertesi gün rakibin kendi aşığını geri getirdi. Mertlik oyundan büyük.",
  ],
  topac_cevirme: [
    "İpi topaca öyle bir sardın ki fırlattığında yerinde çakılı gibi döndü; çocuklar başına toplandı, 'nasıl yaptın' diye. Bir sırrın oldu.",
    "Topacın rakibin topacına çarpıp onu meydandan attı; sevincinden zıpladın. Küçük bir zafer ama tadı akşama dek damağında.",
  ],
  kar_topu: [
    "Duvarın arkasından attığın kar topu tam hedefi buldu; karşı takım dağıldı. Elini üfleye üfleye ısıttın ama gülmen dinmedi.",
    "Küçük bir çocuk kartopundan ağladı; sen yaptığın kaleyi ona bıraktın, elini tuttun. Oyun bitti ama o gün seni unutmadı.",
  ],
  ogul_arisi: [
...
export function resolveMicro(prev: GameState, choice: 0 | 1): GameState {
  ...
  else if (id === "berber_aynasi") { if (choice === 0) { addStatXp(s, "intelligence", 1); p.health = Math.min(100, p.health + 1); } else gainSkill(s, "social", 2); }
  else if (id === "degirmen_sirasi") { if (choice === 0) { p.honor = Math.min(100, p.honor + 1); gainSkill(s, "social", 1); } else { gainSkill(s, "crafting", 1); addStatXp(s, "intelligence", 1); } }
  ...
  else if (id === "dere_bendi") { if (choice === 0) addStatXp(s, "strength", 2); else addStatXp(s, "intelligence", 2); }
  else if (id === "salincak_ipi") { if (choice === 0) addStatXp(s, "stamina", 2); else { addStatXp(s, "intelligence", 2); gainSkill(s, "social", 1); } }
  else if (id === "cember_cevirme") { if (choice === 0) { addStatXp(s, "stamina", 2); addStatXp(s, "intelligence", 1); } else { gainSkill(s, "social", 2); } }
  else if (id === "asik_oyunu") { if (choice === 0) { addStatXp(s, "intelligence", 2); } else { p.honor = Math.min(100, p.honor + 1); gainSkill(s, "social", 1); } }
  else if (id === "topac_cevirme") { if (choice === 0) { addStatXp(s, "intelligence", 1); gainSkill(s, "social", 1); } else addStatXp(s, "stamina", 2); }
  else if (id === "kar_topu") { if (choice === 0) addStatXp(s, "stamina", 2); else { p.honor = Math.min(100, p.honor + 1); gainSkill(s, "social", 1); } }
...
const DIVAN_R_TR: Record<string, [string, string]> = {
  ...
  ac_yil: [
    "Ambarı açtırdın; un çuvalları meydana dizildi, sıra sabaha dek eksilmedi. O kış kimse aç yatmadı; adın 'ekmek veren' diye anıldı, hazine yarıya indi.",
    "Ambarı yaz günü için tuttun; kıtlıkta un pahalıya satıldı. Hazine doldu ama meydandaki bakışlar soğuktu — bir ihtiyar 'padişahın ambarı tok, halkı aç' dedi, söz duvarlara yazıldı.",
  ],
  esir_fidyesi: [
    "Fidyeyi hazineden ödedin; esir düşen sipahiler zincirsiz döndü, anaları eşiğinde ağladı. Askerin gözünde artık 'bizi bırakmayan' oldun.",
    "'Devletin parası fidyeye gitmez' dedin; esirler düşman elinde kaldı. Kese sağlam ama o gün divandan çıkan bir ananın çığlığı kulağında kaldı — bazı hesaplar akçeyle kapanmaz.",
  ],
  sinir_cevizi: [
...
export function resolveDivan(prev: GameState, choice: 0 | 1): GameState {
  ...
  if (id === "ac_yil" && choice === 0 && p.money < 180) return s;
  if (id === "esir_fidyesi" && choice === 0 && p.money < 150) return s;
  s.divan = null;
  ...
  if (id === "su_kavgasi") {
    ...
    if (choice === 0) { p.money -= 180; p.crownAuthority = clamp100(crownAuthorityOf(p) + 7); p.reputation = Math.min(100, p.reputation + 6); bumpNam(p, "comert", 3); p.honor = Math.min(100, p.honor + 3); }
    else { p.money += 90; p.reputation = Math.max(-100, p.reputation - 5); p.fear = Math.min(100, p.fear + 2); bumpNam(p, "zalim", 1); sowSeed(s, { kaynak: "divan_ac_yil", hmin: 24, hmax: 96, agirlik: "orta", nesil: true, etki: { reputation: -4 } }); }
  }
  else if (id === "esir_fidyesi") {
...
export const ACHIEVEMENTS: Achievement[] = [
  ...
  { id: "yediiklim", name: "Yedi İklim",       desc: "On ayrı yerleşime ayak bas.",         icon: "compass",     done: (s) => new Set([...(s.player.cities_visited || []), s.player.location_name]).size >= 10 },
  ...
  { id: "gumus_ocak",  name: "Gümüş Ocak",     desc: "Aynı eşle çeyrek asrı devir.",       icon: "ring",        done: (s) => !!s.player.married && s.player.married_turn !== undefined && (s.turn - s.player.married_turn) >= 300 },
  { id: "altin_ocak",  name: "Altın Ocak",     desc: "Aynı eşle kırk yılı devir.",         icon: "ring",        done: (s) => !!s.player.married && s.player.married_turn !== undefined && (s.turn - s.player.married_turn) >= 480 },
...
export function tendPet(prev: GameState): GameState {
  const s = clone(prev); const p = s.player;
...
export function fundPrestige(prev: GameState, id: string): GameState {
  ...
  if (id === "hekim") { p.health = Math.min(100, p.health + 16); { const hk2 = chance(0.5); push(s, "saglik", hk2 ? `Hekim nabzını defter gibi okudu, üç şişe ve bir perhiz bıraktı; bir haftada belin doğruldu (+16 sağlık).` : `Usta bir hekim tuttun; sağlığın tazelendi (+16 sağlık).`, "kişisel", false, { k: hk2 ? "ev.prestige.hekim2" : "ev.prestige.hekim" }); } }
  else if (id === "imaret") { p.reputation = Math.min(100, p.reputation + 9); p.fame = Math.min(100, p.fame + 3); p.legacy.imaret = true; { const im2 = chance(0.5); push(s, "bagis", im2 ? `İmaretin ilk kazanı şafakta kaynadı; kapıda kuyruk, kuyrukta dua vardı. Aş bitti, bereketi kaldı.` : `Bir imaret açtın; yoksullar adını hayırla anıyor (+itibar).`, "kişisel", true, { k: im2 ? "ev.prestige.imaret2" : "ev.prestige.imaret" }); } }
  else if (id === "hac") { bumpNam(p, "dindar", 16); bumpNam(p, "comert", 6); p.reputation = Math.min(100, p.reputation + 8); p.fame = Math.min(100, p.fame + 4); p.legacy.hac = true; { const hc2 = chance(0.5); push(s, "bagis", hc2 ? `Aylar süren yoldan döndüğünde kapının önü hurma ve zemzem bekleyenlerle doluydu; adının önüne bir hece eklendi: Hacı. O hece, akçeyle alınmayan tek pâyedir.` : `Hacca gidip döndün; artık Hacı diye anılıyorsun, sözün diyarda ağırlık taşıyor (büyük dindar nam + ömür boyu saygınlık).`, "kişisel", true, { k: hc2 ? "ev.prestige.hac2" : "ev.prestige.hac" }); } }
  else if (id === "vakif") { p.reputation = Math.min(100, p.reputation + 12); p.fame = Math.min(100, p.fame + 9); p.legacy.vakif = true; { const vk2 = chance(0.5); push(s, "bagis", vk2 ? `Vakfiye mühürlendi, şartları taşa yazıldı: çeşme akacak, kervan konaklayacak, yetim okuyacak. Sen gittikten sonra da imzan su verecek.` : `Adına bir vakıf kurdun; hayrın nesiller boyu sürecek (+itibar +şöhret).`, "kişisel", true, { k: vk2 ? "ev.prestige.vakif2" : "ev.prestige.vakif" }); } }
  else if (id === "anit") { p.fame = Math.min(100, p.fame + 16); p.legacy.anit = true; { const an2 = chance(0.5); push(s, "bagis", an2 ? `Ustalar son taşı yerine koyduğunda meydan alkışla doldu; kaidede adın, tarihin ve tek satır: bu diyar gördü, bu taş unutmayacak.` : `Görkemli bir anıt diktirdin; diyar bu eseri asırlarca konuşacak (+şöhret).`, "kişisel", true, { k: an2 ? "ev.prestige.anit2" : "ev.prestige.anit" }); } }
  return s;
}

...
export function advance(prev: GameState, n = 1): GameState {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (s.turn <= 3 && !s.player.dead && i === n - 1) {
      ...
      const NBR = ["Komşun sıcak bir çorba ikram etti.", "Pazarda biri eline birkaç akçe sıkıştırdı.", "Anlatılan bir masal yüreğini ısıttı.", "Fırıncı, günün son sıcak somununu eline tutuşturdu.", "İhtiyar bir komşu, kapının önünü süpürürken sana da dua etti."];

...
export default function GameLayout() {
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-200 flex">
      ...
      {/* Mobil alt navigation — sadece 5 ana + "Daha Fazla" */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 border-t border-stone-800 bg-stone-950/95 backdrop-blur flex">
        {visiblePrimary.map((n) => (
          <MobileNavItem
            key={n.to} {...n}
            pulse={
        ...
        <button
          ...
          className="flex flex-col items-center gap-0.5 px-2 py-1.5 text-stone-500 hover:text-stone-300 flex-1"
        >
          <MoreHorizontal className="w-4 h-4" />
          <span className="text-[9px] tracking-wider">Daha Fazla</span>
        </button>
      </nav>

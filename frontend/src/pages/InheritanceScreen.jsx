 ...
 *
 * GameLayout'tan şu şekilde çağrılır:
 *   <InheritanceScreen data={inheritanceData} onSelect={onInherit} busy={inheritanceBusy} />
 *
 * inheritanceData şeması (GET /game/inheritance/options döndürür):
...
export default function InheritanceScreen({
  // GameLayout'tan gelen yeni arayüz:
  data,          // { options, previous_player, generation, inherited_stats, inherited_skills, inherited_money }
  onSelect,      // (childId | null) => void   [GameLayout'un beklediği isim]
  // Eski doğrudan prop arayüzü (geriye uyumluluk):
  playerName: playerNameProp,
  generation: generationProp,
  options: optionsProp,
  inheritedStats: inheritedStatsProp,
  inheritedSkills: inheritedSkillsProp,
  inheritedMoney: inheritedMoneyProp,
  onInherit,     // eski prop ismi
  busy = false,
}) {
  // data prop varsa ondan al, yoksa doğrudan prop'lardan al
  const playerName    = data?.previous_player ?? playerNameProp ?? "Oyuncu";
  const generation    = data?.generation      ?? generationProp  ?? 1;
  const options       = data?.options         ?? optionsProp     ?? [];
  const inheritedStats  = data?.inherited_stats  ?? inheritedStatsProp  ?? {};
  const inheritedSkills = data?.inherited_skills ?? inheritedSkillsProp ?? {};
  const inheritedMoney  = data?.inherited_money  ?? inheritedMoneyProp  ?? 0;
  // İki farklı callback ismini destekle
  const handleConfirmCb = onSelect ?? onInherit ?? (() => {});
  ...
  const handleSelect  = (opt) => { setSelected(opt); setPhase("confirm"); };
  const handleConfirm = () => handleConfirmCb(selected?.id ?? null);
  ...
  return (
    <>
      <style>{`
        ...
        .inh-heir-btn { width: 100%; background: #0e0b07; border: 1px solid #3a2a14; padding: 14px 16px; text-align: left; cursor: pointer; transition: border-color 0.2s, background 0.2s; margin-bottom: 8px; border-radius: 2px; }
        ...
        .inh-btn-primary { width: 100%; padding: 12px; font-family: 'Cinzel', serif; font-size: 11px; letter-spacing: 0.4em; text-transform: uppercase; color: #c8944a; background: #0e0b07; border: 1px solid #c8944a55; cursor: pointer; transition: all 0.2s; margin-top: 16px; border-radius: 2px; }
        ...
        .inh-inheritance-box { background: #0e0b07; border: 1px solid #3a2a14; padding: 16px; margin-top: 12px; border-radius: 2px; }
        .inh-badge { display: inline-flex; align-items: center; gap: 4px; font-family: 'Cinzel', serif; font-size: 10px; color: #c8944a; background: #1e1208; border: 1px solid #3a2a14; padding: 4px 8px; margin: 2px; border-radius: 2px; }
          ...
          {phase === "eulogy" && (
            <>
              <div className="text-center mb-6">
                ...
                <div className="inh-gen" style={{ marginTop: 8 }}>{generation}. Nesil</div>
              </div>
          ...
          {phase === "choice" && (
            <>
              <div className="text-center mb-6">
                ...
                <div className="inh-title" style={{ fontSize: "clamp(18px,4vw,28px)" }}>Varisi Seç</div>
                <p className="inh-quote" style={{ marginTop: 8 }}>Kim bu mirası taşıyacak?</p>
              </div>
            ...
            <>
              <div className="text-center mb-6">
                ...
                <div className="inh-sub">
                  {selected.type === "çocuk"
                    ? `${selected.age} yaşında başlıyor`
                    : "Sürgün · 7 yaşında başlıyor"}
                </div>
              </div>
